...
CATALOG_VERSION = "expanded-safe-presets-v3"
...
def enforce_compatibility(blueprint, character_profile=None):
    ...
    if profile.get("has_skirt_or_robe") and lower in {"side_lunge", "controlled_kick"}:
        item["stride"] = min(item["stride"], 0.105)
        item["lift"] = min(item["lift"], 0.060)
        changes.append("skirt_caps_leg_excursion")
    if profile.get("has_long_sleeve_or_cape") and item["arms"] in WIDE_ARMS:
        item["arm_intensity"] = min(item["arm_intensity"], 0.72)
        changes.append("long_sleeve_caps_wide_arms")
    if profile.get("has_armor_or_bulky_shoulders"):
        item["arm_intensity"] = min(item["arm_intensity"], 0.68)
        if item["arms"] in CROSSING_ARMS:
            item["arms"] = "presentation"
            changes.append("bulky_replaces_crossing_arms")
...
def validate_trajectory(blueprint, frames=181):
    ...
    failures = []
    if report["amplitude_normalized"] < 0.05:
